...
#     count = 0
#     files = [file for file in os.listdir(folder_path) if file.endswith('.gexf')]
#     for file in files:
#         path = os.path.join(folder_path, file)
#         # try:
#         #     logging.info(f"Processing file: {file}")
#         #     graph = nx.read_gexf(path)
#         #     label_mapping = {'benign': 0, 'adware': 1, 'banking': 2, 'sms': 3, 'riskware': 4}
...
#         #         save_data(node_features, api_names, adjacency_matrix,label, label_value, save_path, count)
#         #         logging.info(f"Saved graph {count} to {os.path.join(save_path, f'{label}_{count}.pickle')}")
#         #         count += 1
#         #     else:
#         #         logging.warning(f"Graph {file} resulted in no data to save.")
...
def process_and_save_graphs(folder_path, label, save_path, top_apis, p_nodes):
    ...
    #     futures = {executor.submit(process_file, file, label, save_path, top_apis, p_nodes, label_value, idx + initial_index): idx for idx, file in enumerate(files)}
